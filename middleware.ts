...
export default clerkMiddleware(async (authFn, req) => {
  try {
    ...
    // Check if waitlist redirection is enabled
    const redirectToWaitlist = process.env.NEXT_PUBLIC_REDIRECT_TO_WAITLIST === 'true';

    if (redirectToWaitlist && pathname !== WAITLIST_PATH) {
    ...
    //   if (
    //     publicRoutes(req) &&
    //     !['/sso-callback', '/error'].some(route => req.url.includes(route))
    //   ) {
