...
export default function BookmarksPage() {
  ...
  const getImageUrl = (imageUrl: string | undefined) => {
    ...
    try {
      const { data } = supabase.storage.from("photos").getPublicUrl(imageUrl);
    ...
    try {
      ...
      if (data && data.length > 0) {
        const formattedProjects = data
          .filter(
            (bookmark): bookmark is BookmarkData =>
              bookmark.projects !== null &&
              typeof bookmark.projects === "object"
          )
          .map((bookmark) => ({
            ...bookmark.projects,
            images: bookmark.projects.images
              ? getImageUrl(bookmark.projects.images)
              : "/placeholder.png",
          }));
  ...
  const handleRemoveBookmark = async (projectId: number) => {
    ...
    try {
      ...
      setConfirmRemove({ show: false, projectId: null });
    }
  };

  const confirmRemoveBookmark = (projectId: number) => {
    setConfirmRemove({ show: true, projectId });
  };
  ...
  if (loading) {
    return (
      <div id="loading">
        <Image src={loader} alt="Loading..." className="loader-img" />
      </div>
    );
  }
  ...
  return (
    <>
      ...
      {/* Confirmation Modal */}
      {confirmRemove.show && confirmRemove.projectId !== null && (
        <div 
        className={`fixed inset-0 ${confirmRemove.show ? 'active' : ''} flex items-center justify-center z-50`}
    >
          <div className="confirmation-modal">
            ...
            <div className="confirmation-actions">
              ...
              <button
                onClick={() => handleRemoveBookmark(confirmRemove.projectId!)}
                  ...
                  <div className="card-content">
                    <div className="card-header">
                      ...
                      <div className="card-actions">
                        <button
                          onClick={() => confirmRemoveBookmark(project.id)}
